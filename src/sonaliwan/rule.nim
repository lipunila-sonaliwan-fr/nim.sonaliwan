...
proc newRule*(terms: seq[string], weight: int): Rule =
  result = Rule(terms: terms, weight: weight, id: autoinc)
  index[result.id] = result.terms
  inc autoinc

proc newRuleWithId*(terms: seq[string], weight: int, id: int): Rule =
