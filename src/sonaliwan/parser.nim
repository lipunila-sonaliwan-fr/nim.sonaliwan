    ...
    if ruleParts.len > 2:
      ...
      if key != "#":
        ...
        if key == toLowerAscii(key):
          ...
          discard match(ruleParts[1], re"^([0-9]*)([^0-9]+)$", m)

          if m[1].startsWith("R"):
            result.regexpRules.add(key, rhs)
          else:
            result.terminalRules.add(key, rhs, if m[0].len > 0: parseInt(m[0]) else: 0)
        else:
...
proc getValidTerminalRulesForWords*(
  ...
  for word in words.deduplicate():
    ...
    for (k, terms) in p.terminalRules.rules.pairs:
      let weight = terms.getWeightOfTerm(word)
      if weight > -1:
        validRules.incl k
            ...
            for sdf in p.nonTerminalRules.get(def.terms[rank]):
              ...
              if updDef.len <= wordsSize and processedKeys.inclNotIn(processedKey):
                ...
                if isValid:
                  ...
                  fifoValue.addLast(newRuleWithId(updDef, 0, sdf.id))
...
proc getWeight*(p: Parser, bufferList: seq[string]): seq[string] =
  let reRuleTerm = re"\(([^A-Z\s]+)\s([^A-Zˆ\)]+)\)"
  var bufferMaxR: int = 0
  var bufferRnkg: seq[int] = @[]
  var bufferUnkn: seq[int] = @[]
  for str in bufferList:
    var ranking: int = 0
    var unknown: int = 0
    for ruleTerm in str.findAll(reRuleTerm):
      var m: array[2, string]
      discard match(ruleTerm, reRuleTerm, m)
      let weight = p.terminalRules.get(m[0]).getWeightOfTerm(m[1])
      if weight >= 0:
        ranking += weight
      else:
        inc unknown

    if ranking > bufferMaxR:
      bufferMaxR = ranking

    bufferRnkg.add(ranking)
    bufferUnkn.add(unknown)

  result = @[]
  for ind, str in bufferList:
    var rnk: string = $bufferRnkg[ind]
    var unk: string = $bufferUnkn[ind]
    if unk != "0":
      rnk = rnk & "(" & unk & ")"

    if bufferRnkg[ind] == bufferMaxR:
      rnk = rnk & "\t→ " & str
    else:
      rnk = rnk & "\t  " & str
